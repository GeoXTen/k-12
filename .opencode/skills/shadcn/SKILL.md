---
name: shadcn
description: Manages shadcn components and projects — adding, searching, fixing, debugging, styling, and composing UI, including chat interfaces. Provides project context, component docs, and usage examples. Use when working with shadcn/ui, component registries, presets, --preset codes, or any project with a components.json file. Also triggers for "shadcn init", "create an app with --preset", or "switch to --preset".
---
...
6. **Fix imports in third-party components** — After adding components from community registries, check the added non-UI files for hardcoded import paths. Use `npx shadcn@latest info` to get the correct `ui` alias and rewrite the imports accordingly.
7. **Review added components** — After adding a component or block from any registry, **always read the added files and verify they are correct**. Check for missing sub-components, missing imports, incorrect composition, or violations of the Critical Rules. Also replace any icon imports with the project's `iconLibrary`.
...
**Templates:** `next`, `vite`, `start`, `react-router`, `astro` (all support `--monorepo`), `laravel` (not supported for monorepo)
